...
__global__ void s2g_gpu_gather_kernel(int *in, int *out, int len) {
  ...
  for (int inIdx = 0; inIdx < len; ++inIdx) {
    temp += outDependent(outInvariant(in[inIdx]),inIdx,outIdx);
  }
  out[outIdx] = temp;
}
...
static void s2g_gpu_scatter(int *in, int *out, int len, int nThreads) {
  //@@ INSERT CODE HERE
  //int nThreads = 1024;
  int nBlocks = (len+nThreads-1)/nThreads;
  s2g_gpu_scatter_kernel<<<nBlocks,nThreads>>>(in,out,len);
}

static void s2g_gpu_gather(int *in, int *out, int len, int nThreads) {
  //@@ INSERT CODE HERE
  int nBlocks = (len+nThreads-1)/nThreads;
  s2g_gpu_gather_kernel<<<nBlocks,nThreads>>>(in,out,len);
}
...
int main(int argc, char **argv) {
  ...
  // Utilizing the max possible threads per block and determining
  // block partition size from the problem dimension (see device code)
  cudaDeviceProp deviceProp;
  wbCheck(cudaGetDeviceProperties(&deviceProp, 0));
  int nThreadsPerBlock = deviceProp.maxThreadsPerBlock;
  ...
  s2g_gpu_scatter(deviceInput, deviceOutput, inputLength, nThreadsPerBlock);
  ...
  s2g_gpu_gather(deviceInput, deviceOutput, inputLength, nThreadsPerBlock);
  cudaDeviceSynchronize();
